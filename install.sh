...
if ! grep -q "USER_TIMEZONE=" "${SHELL_RC}" 2>/dev/null; then
    ...
    read -r -p "   IANA timezone (e.g. Asia/Jayapura, Asia/Jakarta, America/New_York) [UTC]: " TZ
    TZ="${TZ:-UTC}"
    {
        echo ""
        echo "# 🕐 Time Context — temporal awareness for AI agents"
...
echo ""
echo "Usage:"
echo "  time_context.py                          # Show current time context"
echo "  time_context.py --format markdown        # Markdown output"
echo "  time_context.py --format json            # JSON output"
echo ""
